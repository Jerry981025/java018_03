...
function rankingStar() {
	...
	axios.get('star')
		.then((res) => {
			...
			let Ranking = Math.round(res.data.rank/res.data.count*10) / 10
			if(res.data.rank === 0){
				$('#c3').text(`${0}(${res.data.count})`)
				$('#c18').text(`${0}(${res.data.count})`)
			} else{
				$('#c3').text(`${Ranking}(${res.data.count})`)
				$('#c18').text(`${Ranking}(${res.data.count})`)
			}
			let starRating = $('#c4');
