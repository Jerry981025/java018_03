...
$(document).ready(function(){
	axios.get('http://localhost:8080/java018_03/RecordsICanHelp').then(res => {
		console.log(res);
		orders = res.data
		let content =``
		for(let i = 0; i < orders.length; i++){
            ...
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse${i}" aria-expanded="false" aria-controls="collapse${i}">
                ...
                <div class="orderRating">獲得評價 </div><div class="stars" id="eachRating${i}"></div>
            </button>
            </h2>
			...
			`
      $('#ICHArea').append(content)
      myRating(orders[i].oRanking, i)
    }
	})
})

function myRating(rating, i){
	...
	for (let k = 1; k <= 5; k++) {
		let stars = rating - k;
		if (stars >= 0) {
			$("#eachRating" + i).append(fullStar)
		} else if (stars < 0 && stars > -1) {
			$("#eachRating" + i).append(halfStar)
		} else {
